...
export class WebSocketService {
  ...
  public DemoBehaviorSubject: BehaviorSubject<any> = new BehaviorSubject("");
  //
    ...
    if ("WebSocket" in window && uuid) {
      ...
      this.ws.onmessage = (evt: any) => {
        try {
          ...
          } else if (received_msg.action === 'buttonClick') {
            console.log("Inside websocket IFELSE Condition",received_msg);
            this.DemoBehaviorSubject.next(received_msg.message);
  ...
  public sendMessage(message: any) {
    ...
    if (this.unauthorized) {
      console.log("Inside WebSocketService : unauthorized");
    }
    let sendFlag = this.ws === undefined ? false : (this.ws.readyState === 1);
    if (message.action !== 'confirmAuth') {
      // console.log("Inside WebSocketService : Not of confirmAuth");
    ...
    if (sendFlag) {
      ...
      message.source = 'webApp';
      this.ws.send(btoa(JSON.stringify(message)));
      // console.log("Inside sendMessage: Message : If  ", message);
    } else {
